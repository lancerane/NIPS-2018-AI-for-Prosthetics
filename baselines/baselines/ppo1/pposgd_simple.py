...
def traj_segment_generator(pi, env, horizon, stochastic):
    ...
    while True:
        ...
        ob, rew, true_rew, new, _ = env.step(ac)
        rews[i] = rew
...
def learn(env, seed, policy_fn, *,
    ...
    if restore_model_from_file:
        ...
        modelF= basePath + '/' + save_prefix +"_afterIter_"+str(load_after_iters) + '.model'
    ...
    while True:
        ...
        if iters_so_far % save_after ==0:
            if save_model_with_prefix:
                basePath=os.path.dirname(os.path.abspath(__file__))
                modelF= basePath + '/' + save_prefix +"_afterIter_"+str(iters_so_far)+".model"
                U.save_state(modelF)
